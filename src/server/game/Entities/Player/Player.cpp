...
void Player::SendSimpleAIOMessage(const std::string &message)
{
    ...
    uint16 parts = ceilf(float(shortMsgLen + 4) / 2600);

    //parts to string
    uint16 high = floorf((float) parts / 254);
    std::string partsStr(1, high + 1);
    partsStr += parts - high * 254 + 1;

    //messageid to string
    high = floorf((float) m_messageIdIndex / 254);
    std::string messageIdStr(1, high + 1);
    messageIdStr += m_messageIdIndex - high * 254 + 1;

    //Increase or renew messageIdIndex 2^16 - 767
    if (m_messageIdIndex >= 64769) {
        m_messageIdIndex = 1;
    } else {
        ++m_messageIdIndex;
    }
