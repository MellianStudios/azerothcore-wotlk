                        ...
                        if ((msg.size() - delimPos - 1) >= 6) {
                            ...
                            if (parts < 2) {
                                sLog->outAIOMessage(
                                    ...
                                    LOG_LEVEL_ERROR,
                                    "HandleAddonMessagechatOpcode: Received AIO addon message with number of parts: %u (< 2). Message Id: %u, Sender: %s",
                                    parts,
                                    messageId,
                            ...
                            if (parts > maxparts) {
                                sLog->outAIOMessage(
                                    sender->GetGUID().GetCounter(),
                                    LOG_LEVEL_ERROR,
                                    "HandleAddonMessagechatOpcode: Received AIO addon message with too many parts: %u (> %u). Message Id: %u, Sender: %s",
                                    parts,
                                    maxparts,
                                    messageId,
                                    sender->GetName().c_str()
                            ...
                            if (messagePartsItr == _addonMessageBuffer.end()) {
                                messagePartsItr = _addonMessageBuffer
                                    .insert(std::make_pair(messageId, LongMessageBufferInfo())).first;
                            } else {
                            ...
                            if (messagePartsItr->second.Map.size() >= messagePartsItr->second.Parts) {
                                ...
                                for (AddonPartStringMap::const_iterator itr = messagePartsItr->second.Map.begin();
                                     itr != messagePartsItr->second.Map.end(); ++itr) {
                            ...
                            sLog->outAIOMessage(
                                sender->GetGUID().GetCounter(),
                                LOG_LEVEL_ERROR,
                                "HandleAddonMessagechatOpcode: Received AIO long addon message without meta data, Sender: %s",
                                sender->GetName().c_str()
                            );
